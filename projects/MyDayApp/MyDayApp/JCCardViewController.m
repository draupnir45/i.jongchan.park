...
- (UIView *)fetchNewCardWithView:(UIView *)view {
    //뷰를 이용해 카드를 만듭니다.
    ...
    UIView *card = [[UIView alloc] initWithFrame:CGRectMake(15, 0, self.frameSize.width - 30, view.frame.size.height)];
...
- (UIView *)changeContentOfCardWith:(UIView *)view toView:(UIView*)motherView {
    if (view.frame.size.height > 200) {
        [view setFrame:CGRectMake(0, 0, self.frameSize.width-30, view.frame.size.height)];
    } else {
        [view setFrame:CGRectMake(0, 0, self.frameSize.width-30, 200)];
...
- (UIView *)cardForDepthAtIndex:(NSInteger)index {
    ...
    if (card == nil) {
        card = [self fetchNewCardWithView:self.dataArray[index]];
    } else {
        card = [self changeContentOfCardWith:self.dataArray[index] toView:card];
    }
...
- (void)dequeueReusableCards {
    ...
    UIView *firstCard = [self.displayingViewsArray firstObject];
    NSInteger numberOfReusableCards = (NSInteger)((self.scrollView.contentOffset.y - firstCard.frame.origin.y - self.rowHeight*self.numberOfCardsInFrame) / self.rowHeight);
    
    
    if (numberOfReusableCards > 1) {
        NSArray *itemsToDeque = [self.displayingViewsArray subarrayWithRange:NSMakeRange(0, numberOfReusableCards)];
        
        for (NSInteger i = 0; i < itemsToDeque.count; i++) {
            [self.displayingViewsArray removeObject:itemsToDeque[i]];
            [itemsToDeque[i] removeFromSuperview];
            [self.dequeuedArray addObject:itemsToDeque[i]];
            NSLog(@"아이템이 디큐됩니다.dataArray[%ld]",(long)i);
        }
...
- (void)initialQueue {
    ...
    self.scrollView.contentSize = CGSizeMake(self.scrollView.contentSize.width, self.rowHeight * self.dataArray.count);
}
